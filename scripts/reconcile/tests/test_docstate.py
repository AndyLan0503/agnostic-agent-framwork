...
from reconcile.docstate import DocState, Record, classify, load, write
from reconcile.judge import VerdictKind
...
class ReadWriteTest(unittest.TestCase):
    def test_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            write(root, state)
            self.assertTrue((root / ".docstate").exists())
            back = load(root)
...
class SyncTest(unittest.TestCase):
    ...
    def test_sync_is_idempotent(self):
        with tempfile.TemporaryDirectory() as tmp:
            repo = Repo(Path(tmp))
            sync(repo.root)
            first = (repo.root / ".docstate").read_text()
            sync(repo.root)
            self.assertEqual(first, (repo.root / ".docstate").read_text())
