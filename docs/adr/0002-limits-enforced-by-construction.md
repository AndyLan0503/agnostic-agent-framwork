...
Unattended runs (Level 2) launch only through `scripts/gnhf.py`, whose
...
- A PreToolUse guard (`scripts/gnhf_guard.py`) that fires regardless of
  ...
  for local-only subcommands (knowledge/gnhf-safe-subcommands.md).
