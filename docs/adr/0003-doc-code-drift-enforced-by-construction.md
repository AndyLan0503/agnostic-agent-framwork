...
agent cooperation. The collaboration-artifacts convention - `knowledge/` cards
and `docs/specs/` stay in sync with the code they describe, updated in the same
...
Python subpackage (`scripts/reconcile/`), and add its check as the enforcement
...
any CI wiring. The M0 schema (`docs/specs/reconcile.md`) is the contract every
