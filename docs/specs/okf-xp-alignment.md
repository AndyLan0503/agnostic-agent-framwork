...
1. **OKF (Open Knowledge Format).** The `knowledge/` corpus is already
   OKF-shaped: markdown + YAML frontmatter, one fact per card, cross-linked,
   git-native (`knowledge/README.md` card format; every card carries
...
2. **XP enforcement.** The repo already encodes XP disciplines piecemeal:
   ...
   (AGENTS.md Guardrails) is test-first/TDD; `roles/implementer.md` step 1
   "Work test-first"; guardrail 5 "Red is fixed now" is continuous
   integration discipline; `roles/interrogator.md` "Unnecessary complexity -
   ...
   the repo's own thesis (docs/adr/0002: "a rule that exists only in prose is
...
- **As an implementer-role agent**, I want XP practices that apply to my work
  ...
  obligations in `roles/implementer.md`, backed by a mechanism where one
...
- **As a maintainer**, I want each new claim tied to a file path, so the
  drift reconciler (docs/adr/0003) can eventually govern it.

### Acceptance criteria (Given/When/Then)

- **OKF naming.** Given the `knowledge/` corpus, When a reader opens
  `knowledge/README.md`, Then it states that the cards follow OKF (Open
...
- **OKF card exists.** Given the knowledge base, When a reader greps for the
  ...
  `knowledge/README.md` and, if written, the OKF ADR) and appears in the
...
- **XP in the implementer role.** Given `roles/implementer.md`, When an
...
- **No regression.** Given the change, When `make test` runs, Then it stays
  ...
  (knowledge/shim-files-point-to-agents-md.md).

### In scope

- Naming the OKF alignment in `knowledge/README.md` and recording it as a
  knowledge card in the Index.
- Naming XP and mapping its practices in AGENTS.md (Guardrails / Enforcement
  map / Roles / Conventions).
- Stating XP obligations as checkable steps in `roles/implementer.md` within
...
- Changing the drift reconciler (docs/adr/0003) or its schema; OKF/XP cards
...
- **Guardrail 3** (agents never commit/push/merge/deploy on their own) - XP's
  ...
  no-commit.
- **docs/adr/0002** (limits enforced by construction, not prose) - any XP
...
- **docs/adr/0001** (tool-neutral source of truth; shims are pointers) - all
  XP/OKF guidance lands in AGENTS.md / `roles/` / `knowledge/`, never in a
  harness shim (knowledge/shim-files-point-to-agents-md.md).
- **knowledge/README.md conventions** - one fact per card; the OKF card must
  be a single fact, and any field rename must keep `sources` pointing at code
  that proves the claim.
- **docs/specs/README.md** - "A decision that is expensive to reverse gets
...
1. **Does OKF alignment warrant its own ADR?** A field-name migration and an
   external-standard commitment are expensive to reverse (docs/adr/README.md:
...
2. **Should the card format adopt OKF's reserved field names?** Three
   ...
   card in `knowledge/` (currently 3) plus `knowledge/README.md` plus any
...
4. **XP: documentation-only or new mechanisms?** Which practices are
   ...
   - *Aspirational (no cooperation-free mechanism today):* merciless
     ...
     finding) warranted? Per docs/adr/0002 they cannot be called "guardrails"
     without one.
...
1. Conform the knowledge-card format to OKF (Open Knowledge Format), doing a
   ...
   recorded in EXISTING documents - `knowledge/README.md` (the card-format
...
2. Unify the two frontmatter formats: declare the reconciler's `reconcile:`
   ...
   file under `scripts/reconcile/` changed" and "`reconcile:` block
   byte-unchanged": `scripts/reconcile` changes and card `reconcile:` bindings
   are now IN SCOPE.
...
- A new ADR. The OKF conformance decision and field-mapping table are recorded
  in `knowledge/README.md`, AGENTS.md Conventions and the OKF card (Change 1),
...
- Any reconciler behavior change beyond what unification requires. In scope:
  verifying/extending `scripts/reconcile/frontmatter.py` to parse OKF-format
  ...
  writer, `apply`, judge, or CI wiring (docs/adr/0003 M2+).
...
- **P0-1. Record the OKF decision + field mapping in existing docs (no ADR).**
  ...
  that already exist:
  - `knowledge/README.md` (the card-format authority) gets the OKF subsection
  ...
  There is no `docs/adr/0004-*.md` file, no `docs/adr/README.md` row for it,
  ...
  Field mapping (the exact contract `knowledge/README.md` records):
...
- **P0-2. Card format doc + OKF subsection (`knowledge/README.md`).** Rewrite
...
- **P0-3. Migrate every card + add the OKF card.** Apply the rename to all
  ...
  `knowledge/knowledge-cards-follow-okf.md` (`type: convention`) recording the
  OKF-alignment fact, with `sources` pointing at `knowledge/README.md` (the
  authority - no ADR to cite), and add it to the `knowledge/README.md` Index.

- **P0-4. Conformance test (guardrail 2 - behavior lands with tests).** Add a
  stdlib-only `scripts/test_knowledge_cards.py` (unittest, discovered by
  `make test`) that, for every `knowledge/*.md`:
...
*Unification + drift-govern (Change 2 - `scripts/reconcile` now in scope):*
...
- **P0-5. Unify the frontmatter (`reconcile:` is an OKF extension field).**
  ...
  (the nested `reconcile:` block). Document this in BOTH `knowledge/README.md`
  (the OKF subsection, P0-2) and `docs/specs/reconcile.md` (a note that the
  `reconcile:` block is one field of an OKF-format document, coexisting with
  OKF scalars). Then make the parser honest about it:
  - Verify `scripts/reconcile/frontmatter.py` cleanly parses OKF-format cards -
...
- **P0-6. Drift-govern the knowledge cards.** Add a `reconcile:` binding to the
  ...
  **Which cards get a binding (state the decision):** EVERY sourced card gets a
  ...
  `docs/adr/0001-*.md`; `shim-files-point-to-agents-md` -> the shim files) the
  reconciler simply hash-gates / degrades to whole-file (docs/specs/reconcile.md:
  "non-Python file ... degrades to the whole file"), which is fine - it still
  catches gross drift. The new OKF card's `sources` is `knowledge/README.md`
...
- **P0-7. Enforcement-map rows (only the three with a real mechanism).** Add
  ...
  mechanism, a row would itself be a wish (docs/adr/0002).
...
- **P0-8. Name XP in AGENTS.md + OKF Conventions note, additively.** Add a new
  ...
  - merciless refactoring, collective ownership, YAGNI-as-gate -> labelled
    CONVENTIONS (no cooperation-free mechanism today, per docs/adr/0002); name
  ...
  authority is `knowledge/README.md`.

- **P0-9. Implementer XP obligations (`roles/implementer.md`).** Restate the
...
seeding / `sync` / `apply` for the new card bindings (docs/specs/reconcile.md
...
- `make test` green after the change, including the new
  `scripts/test_knowledge_cards.py` AND the existing reconcile subpackage tests
...
- Every `knowledge/*.md` carries a `type` in `{convention, mechanism}`; no card
  carries `updated`. (Enforced by P0-4, not just observed.)
- `knowledge/README.md` names OKF, its reserved set, the required field, the
  ...
  not in an ADR).
- No `docs/adr/0004-*.md` file exists; no `docs/adr/README.md` row for it; no
...
- `docs/specs/reconcile.md` notes that `reconcile:` is one field of an
  OKF-format document.
...
block (docs/adr/0003) that `scripts/reconcile/frontmatter.py` parses as its own
...
to `knowledge/README.md` (format + OKF subsection), 3 migrated + drift-governed
cards, 1 new OKF card, 1 new `scripts/test_knowledge_cards.py`; a note in
`docs/specs/reconcile.md`; a possible small change to
`scripts/reconcile/frontmatter.py` plus one new fixture and one new
`test_frontmatter.py` case; additive edits to AGENTS.md (XP subsection, 3
enforcement rows, OKF Conventions note) and `roles/implementer.md`. No change to
`scripts/gnhf*` or the reconciler's runtime behavior beyond frontmatter parsing.
...
because the decision lives in `knowledge/README.md` (not an append-only ADR),
...
- Whole-file bindings on doc-sourced cards degrade coarsely (no symbol-level
  ...
  none, and matches docs/specs/reconcile.md's stated degrade-to-whole-file
...
- **OKF conformance** (the field-mapping table, `type` as the sole required
  ...
  `knowledge/README.md` (the card-format authority), with a naming note in
  ...
  scoped it to the format authority document; `knowledge/README.md` is the
...
- **Unification of the two frontmatter formats** (`reconcile:` as an OKF
  ...
  `knowledge/README.md` and `docs/specs/reconcile.md`, enforced by the parser +
...
- **XP framing** is additive documentation mapping existing mechanisms; it
  ...
  `roles/implementer.md`, not an ADR. (Resolves open question 5: additive
  only.)
...
1. **Format + decision docs first (no ADR).** Rewrite `knowledge/README.md`
...
2. **Conformance test + card migration + reconcile bindings.** Add
   `scripts/test_knowledge_cards.py` (P0-4) as failing tests, then migrate all
...
3. **Reconcile frontmatter unification + reconcile test.** Verify/extend
   `scripts/reconcile/frontmatter.py` to parse OKF-format cards, add the
   full-OKF-card fixture and the `test_frontmatter.py` case, and add the note
   to `docs/specs/reconcile.md` (P0-5 code half). Existing reconcile subpackage
   tests stay green.
...
5. **Implementer obligations.** Restate XP obligations as checkable steps in
   `roles/implementer.md` (P0-9), adding no autonomy.
