...
- claude: headless Claude Code with the gnhf settings profile - prompts
  ...
  framework/scripts/gnhf_guard.py hook. Resumes the same session
...
Usage: python3 framework/scripts/gnhf.py "the feature request" [--harness codex]
"""
...
START_PROMPT = "Follow framework/skills/unattended-run/SKILL.md for this request: {request}"
CONTINUE_PROMPT = (
    "Continue the unattended run per framework/skills/unattended-run/SKILL.md for this "
...
def build_command(request: str, resume: bool, settings: str,
    ...
    if resume:
        prompt = "Continue the unattended run per framework/skills/unattended-run/SKILL.md."
