"""Parse the narrow `reconcile:` frontmatter and anchor fences.

Stdlib-only: a tolerant hand-written parser for this one fixed shape, not a
general YAML engine (framework/docs/adr/0003 - stdlib-first). A `.md` with no
`reconcile` frontmatter is unmanaged and ignored.
"""
...
def parse_frontmatter(text: str) -> ManagedDoc | None:
    """Return the ManagedDoc, or None if the doc carries no `reconcile:` block.
    ...
    # Find the `reconcile:` top-level key and its indented body.
    body: list[str] = []
    in_reconcile = False
    base_indent = 0
    for line in lines:
        if not line.strip() or line.lstrip().startswith("#"):
            ...
            continue
        if not in_reconcile:
            if re.match(r"^reconcile\s*:", line):
                in_reconcile = True
    ...
    if direction is None:
        return ManagedDoc(
            direction=None,
            error="managed doc missing `direction` in reconcile frontmatter",
        )
