"""`reconcile apply`: fix drift in the SAFE direction only (framework/docs/adr/0003, M3).
...
apply requires recorded state; without a `.docstate` it refuses (run `sync`).
"""
...
def apply(root: Path, generator: Generator | None = None) -> ApplyResult:
    ...
    if recorded is None:
        return ApplyResult(refused=[Refusal(
            "*", "no .docstate lockfile; run `reconcile sync` first")], ok=False)
    ...
    for r in res.resolved:
        ...
        if rec is None:
            result.surfaced.append(Refusal(
                r.entry_id, "unrecorded binding; run `reconcile sync` first"))
            continue
        ...
        if structural is VerdictKind.DOC_DRIFT:
            result.surfaced.append(Refusal(
                r.entry_id, "doc-drift: confirm the edit via `reconcile sync`"))
            continue
