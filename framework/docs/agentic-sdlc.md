...
live in `framework/roles/`; this document is the pipeline that connects them.
...
2. **Gates are mechanical.** Commit, push, merge and deploy are human
   ...
   allowlist. An agent finishing a phase stops and reports;
...
request
  ...
  -> business-analyst    requirements + acceptance criteria   -> framework/docs/specs/<feature>.md  ## Analysis
...
| 1 - conducted | One main agent session dispatches roles and threads artifacts, per `framework/skills/conduct-pipeline/SKILL.md` (Claude Code binding: `/ship`) | at every human gate | real feature work (default) |
| 2 - headless | `framework/scripts/gnhf.py` launches a contained session running `framework/skills/unattended-run/SKILL.md` (Claude Code binding: `/gnhf`) | none; assumptions recorded, checkpoints committed locally on a `gnhf/` branch, never pushes | overnight runs, after Level 1 is trusted |
...
windows and resumes. Use it only after Level 1 is trusted
in the adopting project.

Practical usage of both levels - invocation, gates, steering, the morning
review ritual - is in framework/docs/running-the-pipeline.md.
