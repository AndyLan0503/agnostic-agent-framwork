...
Two ways to run the same pipeline (framework/docs/agentic-sdlc.md). `/ship` keeps you
...
Any other harness: "Follow framework/skills/conduct-pipeline/SKILL.md for: <request>".
...
- The spec file is the steering wheel. If implementation is drifting, stop,
  edit `framework/docs/specs/<feature>.md`, and redispatch the implementer - don't
...
```
python3 framework/scripts/gnhf.py "add CSV export to the reports page; \
...
- **Terminal**: `python3 framework/scripts/gnhf.py "<request>"` from the repo root.
...
Containment: edits confined to the repo, nothing remote,
