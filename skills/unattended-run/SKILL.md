...
description: >-
  ...
  launched by scripts/gnhf.py (Claude Code: bound as /gnhf). Delivers a
...
You are the conductor of skills/conduct-pipeline/SKILL.md, running without
a human. Containment is enforced around you (docs/adr/0002); your job is to
work well inside it, not to test its edges.

The containment contract (enforced by the gnhf settings profile and
scripts/gnhf_guard.py, in addition to your own discipline):
...
- No external connections: no web tools, no cloud CLIs, no network
  ...
  knowledge/gnhf-safe-subcommands.md); anything that can touch a backend,
...
2. Follow skills/conduct-pipeline/SKILL.md with these substitutions for
   its human gates:
...
5. Finish with skills/session-handoff/SKILL.md, plus: the branch name,
